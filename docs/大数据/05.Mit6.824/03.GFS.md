...
log+checkponit ->stabblestorge   日志和检查点需要持久化存储，这样当master奔溃时可以恢复。

> Master会定期存储存储点
>
> 日志中还会存储ichunk handle列表（就是要获取的chunk handle列表）

Q:从文件名到chunk handle的映射需要持久化存储嘛

A:no，因为日志中有chunk handle的列表，可以去chunk server再过得文件名 到handle的映射

Q：chunk handle 到 chunk server的映射需要持久化保存嘛？

A:no，因为master如果崩溃了，再启动时要求server告诉他，server拥有的块

Q:chunk handle的version需要持久化存储吗？
