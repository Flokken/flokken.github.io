...
- `SCOPE_SINGLETON`、`SCOPE_PROTOTYPE`，对象类型的创建获取方式，主要区分在于 `AbstractAutowireCapableBeanFactory#createBean` **创建完成对象后是否放入到内存中，如果不放入则每次获取都会重新创建**。
  - 放入内存就是放入**缓存**，如果是单例，我们每次就是用的一开始创建那个Bean
...
- **所以非 Singleton 类型的 Bean 不需要执行销毁方法**
- 所以这里的代码会有两处修改
...
>上面的内容是在spring中增加对Bean是单例还是原型的判断和处理逻辑

### 定义 FactoryBean 接口

> FactoryBean体系是为了让用户可以实现自己创建Bean对象并且交给spring管理
>
> 用户实现的Bean对象实现FactoryBean接口即可
...
<beans>
    ...
    <bean id="userService" class="com.inet.bean.UserService"   scope="prototype">
    ...
    <bean id="proxyUserDao" class ="com.inet.bean.ProxyBeanFactory"/>
...
- 关于 FactoryBean 的调用并没有太多不一样，因为所有的不同都已经被 spring.xml 配置进去了。所以这里直接像Spring创建的userDao一样的，直接调用代理方法即可
...
- bean对象不仅仅只有单例，可能还会是原型，所以需要判断Bean是单例还是原型
- 并且使用者应该可以自己定义并注册复杂或者**动态代理**的Bean对象到Spring中，满足扩展组件开发的需求。

### Task：

- 增加Bean的作用域，不仅仅是单例。

- 实现FactoryBean，使得用户可以自己定义Bean对象让spring管理

### Action

bean的作用域：

- BeanDefinition类中增加作用域相关字段 
- 加载解析资源时增加解析作用域参数 
- `AbstractAutowireCapableBeanFactory`类中生成bean时进行判断，若是BeanDefinition标记为单例模式的bean放入单例缓存池中，原型模式的bean则每次重新生成
...
- 创建 FactoryBeanRegistrySupport抽象类，继承自 DefaultSingletonBeanRegistry   

  - 内部持有一个由FactoryBean提供给**使用者自定义生成bean构建出的对象的缓存池**即`factoryBeanObjectCache`

    - ```java
      private final Map<String, Object> factoryBeanObjectCache = new ConcurrentHashMap<String, Object>();
      ```
  
    - 当然，这个缓存池存的是用户自己创建的单例Bean（因为只有单例Bean才需要保存）
  
  - 根据beanName获取缓存池中的对象  
  
  - 根据factoryBean和beanName获取生成的对象，若是缓存池中没有bean则进行生成，**同样会判断是否时单例bean，若是则生成后放入缓存池中**
  
  - 调用实现FactoryBean构建对象的方法，生成时实则调用本方法

> 这个getObject的具体内容是用户实现，因为用户实现这个接口就是为了自定义Bean，而不是只能通过spring来创建Bean
