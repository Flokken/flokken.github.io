...
$f[i+1][j+1]=max(f[i][j+1],f[i+1][j],f[i][j])+1,s[i]!=t[j]$
...
<img src="C:/Users/28788/AppData/Roaming/Typora/typora-user-images/image-20240407193911546.png" alt="image-20240407193911546" style="zoom:80%;" />
...
class Solution {
    ...
    public int dfs(int i,int []nums){
        ...
        for(int j=0;j<i;j--){
           if(nums[i]>nums[j]){
              res=Math.max(res,dfs(j,nums));//res是剩余子数组的最大值
           }
        }
        return vis[i]=res+1;//这里的1表示nums[i]
    }
}
```
